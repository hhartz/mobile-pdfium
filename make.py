...
def run_task_build_ios(ios_archs, ios_configurations):
    ...
    for config in ios_configurations:
        ...
        for arch in ios_archs:
            ...
            args = 'target_os="ios" target_cpu="{0}" use_goma=false is_debug={1} pdf_use_skia=false pdf_use_skia_paths=false pdf_enable_xfa=false pdf_enable_v8=false pdf_is_standalone=true is_component_build=false clang_use_chrome_plugins=false ios_enable_code_signing=false enable_ios_bitcode=false symbol_level=0 use_sysroot=false use_system_zlib=true use_partition_alloc=false pdf_is_complete_lib=true use_allocator_shim=false use_xcode_clang=true'.format(arch, arg_is_debug)
